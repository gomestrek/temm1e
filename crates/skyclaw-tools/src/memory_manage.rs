...
impl MemoryManageTool {
    ...
    async fn handle_remember(
        ...
        let key = input
            .get("key")
            .and_then(|v| v.as_str())
            .ok_or_else(|| SkyclawError::Tool("Missing required parameter: key".into()))?;
        let content = input
            .get("content")
            .and_then(|v| v.as_str())
...
mod tests {
    use super::*;
    use skyclaw_test_utils::MockMemory;
    ...
    async fn recall_by_key() {
        ...
        let input = make_input(serde_json::json!({
            "action": "remember",
            "key": "project",
            "content": "Working on SkyClaw v1.4",
            "tags": ["dev"]
        }));
        tool.execute(input, &ctx).await.unwrap();
        ...
        assert!(!output.is_error);
        assert!(output.content.contains("SkyClaw v1.4"));
    }
